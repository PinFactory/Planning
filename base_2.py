...
class cEffect:
	def __init__(self, Name, bActive = True):
		self.name = Name
		self.curReserveTime = 0
		self.bActive = bActive
		for iText in arrEffectText:
			if iText[0] == Name:
				self.reserveTime = iText[1]
				return
		print 'not mateched arrEffectText data' + Name		

	def activeEffect(self):
		self.bActive = True

	def runReserveTime(self):
		if self.bActive == False:
			return
		self.curReserveTime -= 1
		if self.curRserveTime <= 0:
			self.curRserveTime = self.reserveTime
			self.bActive 		= False

	def reduceReserveTime(self, time):
		if self.bActive == False:
			return
		self.curReserveTime -= time
		if self.curRserveTime <= 0:
			self.curRserveTime = self.reserveTime
			self.bActive 		= False
...
class cState:
	...
	def removeProperty(self, _rmProperty):
		for iProperty in self.property:
			if iProperty.getName() == _rmProperty:
				del iProperty

	def addProperty(self, _addProperty):
		self.property.append(cEffect(_addProperty))	
	...
	def IsMatchedCondition(self, _Condition):		
		if _Condition == 'backPosition':
			...
			for iProperty in self.property:
				if _Condition == iProperty.getName():
					return True
		return False
	...
	def reduceReserveEffectTime(self, time):
		for iProperty in self.property:
			iProperty.reduceReserveTime(time)
			if iProperty.IsActive() == false:
				del iProperty



arrText = [
# name 			cooltime   action   positive effect								pos effect 						negeffect
...
['thunder',		130,		2,		['hide','shortDistance'],				['blade','stun'],				[]],
['hot_Wind',	150,		5,		['stun','shortDistance'],				['air1','blade'],						[]],
['amyun',		100,		8,		['air1','shortDistance'],				['air2','blade'],						[]],
['bomb',		180,		1,		['air2','shortDistance'],				['air2','bomb_flag'],			[]],
['spider',		120,		40,		['air2','shortDistance'],				['down','longDistance'],		[]],
['fire',		100,		1,		['down','bomb_flag'],					['stun'],						['bomb_flag']],
['backstep',	120,		4,		['shortDistance'],						['longDistance'],				[]],
['shadowBlade',	200,		2,		['blade','stun'],						['shadow_flag'],				[]],
['shadowStep',	100,		3,		['shortDistance','shadow_flag'],		['hide'],						['shadow_flag']],
]
...
class cEnvironment:
	...
	def applyAction(self, _ActionName, bUsed = False, bApplyActionTime = False, bWriteLog = False):
		...
		if bUsed is True:
			action.setUsed()
		if bWriteLog is True:
			self.history.append(_ActionName)
...
def run(bPrint = False, bPrintHistory = False):
	if frontier.empty() == False:
		...
		if len(possibleAction) == 0:
			if bPrint == True:
				current.printState()
			if bPrintHistory == True:
				current.printHistroy()
		#print possibleAction	
		for iAction in possibleAction:
			next = copy.deepcopy(current)		
			next.applyAction(iAction,bUsed = True, bApplyActionTime = True, bWriteLog = True)			
			frontier.put(next)
		return True
	else:
		return False
...
'''
run(bPrintHistory = True)
run(bPrintHistory = True)
run(bPrintHistory = True)
run(bPrint = True, bPrintHistory = True)
run(bPrint = True, bPrintHistory = True)
run(bPrint = True, bPrintHistory = True)
run(bPrint = True, bPrintHistory = True)
run(bPrint = True, bPrintHistory = True)
...
for i in range(1000):
	if run(bPrintHistory = True) == False:
